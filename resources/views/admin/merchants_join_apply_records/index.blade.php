...
@section('title', '加盟申请列表')
@section('breadcrumb')
    {!! Breadcrumbs::render('join_apply_records'); !!}
@endsection
@section('style')
...
@section('content')
            ...
            <div class="panel panel-default" data-sortable-id="table-basic-2">
                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-12 col-sm-12">
                            <form action="{{ route('join_apply_records.index') }}" class="form-inline pull-right">
                                <!-- 状态 -->
                                <div class="form-group form-group-sm">
                                    <select name="status" class="form-control">
                                        <option value="">请选择状态</option>
                                        @foreach(\App\Models\MerchantsJoinApplyRecord::$statusText as $k => $item)
                                            <option value="{{ $k }}"
                    ...
                    <table class="table table-hover">
                        ...
                        @if(count($lists))
                            @foreach($lists as $item)
                                <tr>
                                    ...
                                    <td>{!! $item::$typeStoreText[$item->type] !!}</td>
                                    <td>{!! $item::$levelText[$item->level] !!}</td>
                                    <td>{{ $item->recommend_user->name ?? '-' }}</td>
                                    <td>{{ $item->contact ?? '-' }}</td>
                                    <td>{{ $item->phone ?? '-' }}</td>
                                    <td>{!! $item::$statusText[$item->status] !!}</td>
                                    <td>{{ $item->created_at }}</td>
                                    <td>
                                        <a href="{{ route('join_apply_records.edit', ['id'=>$item->id]) }}"
                                           data-toggle="modal"
                        ...
                        @else
                            <tr>
                                <td colspan="10" align="center"><i class="fa fa-info-circle"></i>&nbsp;暂无符合条件的记录</td>
                            </tr>
                        @endif
    ...
    <div class="modal modal-message fade" id="modal-default-edit">
        <div class="modal-dialog">
            <form class="form-horizontal" action="{{ route('join_apply_records.update') }}" id="edit" method="post"
